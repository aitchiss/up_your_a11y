...
category: 'forms'
description: 'This topic introduces the key concepts to creating simple, accessible forms including labelling & describing inputs, and organising your form.'
accentColor: '#FFE1F4'
keyTakeaways:
...
You will notice that although this looks OK on the page, when you tab to the input there is very little information provided; on VoiceOver it simply says "edit text, blank". This can easily be remedied by using the appropriate label element, and ‘for’ tag, as in the code example below. By adding this label, VoiceOver will now announce "First name, edit text". The form input rendered by this example is also included below so you can turn on your screen reader and hear the difference:
...
The above effect can easily be achieved accessibly, by either positioning a label hidden off-screen using CSS, or more simply by using the aria-label attribute:
...
Consider also adding descriptive text alongside your label/input communicating clearly any special requirements for the user input. For example:
...
Now you have the basics in place, check out <Link to="/handling-form-errors">Handling Validation Errors in Forms</Link>, which will give you some tips on how to handle failed input validation and errors. Keep an eye out for more advanced Form topics on Up Your A11y coming soon, and in the meantime, check out the further reading suggestions below.
