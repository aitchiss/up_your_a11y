...
- Utilising the “aria-describedby” property to provide more detailed information about expected input types/patterns/validations
...
Keeping in mind that screen reader users are likely to complete forms by tabbing through inputs and to the submit button, turn on your screen reader and try tabbing to the email input above, and then (without entering any data) to the submit button. What do you notice?

The main problems can be summarised as:

1. Feedback is provided visually (red highlight, error message) but is not announced to the user
2. Submit button cannot be tabbed to because it has the “disabled” attribute

This simple example has only one input, with a very simple validation, but you can easily imagine the confusion faced by a user if they have completed multiple form inputs and then arrive at the bottom of the page without any feedback or a submit button.
...
- rather than disabling the submit button, perform validation on all inputs when the button is clicked
- If any of the inputs are invalid, we add the "aria-invalid" attribute, prevent the form from being submitted, and
- We display any and all errors in a list on the same page
...
The implementation for the example above would be:

<ExampleFormContainer>
  <ErrorFormWithList />
</ExampleFormContainer>

Using your screen reader, try tabbing to and clicking the submit button with no input in the email field.
...
- As long as a clear message is input into the error list (including the name of the field where each error has occurred, and a clear description of what is missing), it provides a very helpful and concise reference for users
...
Based on these pros and cons, the ‘error list’ approach might therefore be appropriate in use cases where
...
- If the input is invalid, we will set the 'aria-invalid' attribbute, add an error message directly below the input, and announce the error to the user
- Remove any input’s associated error message a soon as its value is changed
...
This approach also makes use of the “alert” role, but on an input-by-input basis. Try turning on your screen reader and checking the example below. Tab to the email input, and then immediately tab to the submit button without entering anything in the input:
...
- The same validation needs to take place on clicking the submit button, as input-by-input validation takes place in the ‘onBlur’ event and most users (especially those not using a screen-reader) may click submit before the final input is blurred
...
- Error messages follow input-by-input and can be easier to locate after they are announced. If the form does not already make use of “aria-describedby” elements, then the attribute can be added to the error message, directly linking it to the input (as in the example above)
