...
<DecorativeList bulletsContent={['List Item One', 'List Item Two']} />
...
Here you can see that we are using the image like a title, with the description below going on to explain the ingredients or recipes steps required to make the item. In this case the alt text could usefully just be the name of the object pictured, i.e. "Cherry Pie".
...
In this case, we can see that the detail of the image is central to conveying meaning. In this case, we would want to provdie more detail in the alt text, e.g. "A slice of cherry pie served on a white plate with two fresh cherries beside it"
...
The final question is the difficult one to answer, but by taking a little time over this, you can improve the experience for all of your users, as well as your SEO!
