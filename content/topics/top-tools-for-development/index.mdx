...
Accessibility Support is very similar in principle to the much used <a href="https://caniuse.com/">Can I Use</a>, only for accessibility specifically. You can search for a particular element or attribute, and get a quick rundown of which assistive technologies are known to support it. It's a great time saver to prevent cross-platform issues, e.g. if a feature is supported in VoiceOver on MacOS but not on NVDA/Windows. The project is community-drive and open-source, so it’s easy to raise an issue or contribute if you can.
